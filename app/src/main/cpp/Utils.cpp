...
void initAppEnv(JNIEnv *env, jobject context) {

    jmethodID methodGetDeviceID = env->GetStaticMethodID(baseClasses.jniUtilClass,
                                                         SecureUtil_getDeviceId_Method,
                                                         SecureUtil_getDeviceId_Param);
    jstring deviceId = (jstring) env->CallStaticObjectMethod(baseClasses.jniUtilClass,
                                                             methodGetDeviceID,
                                                             context);
    string strDeviceId = jstring2String(env, deviceId);

    jmethodID methodGetAppVersion = env->GetStaticMethodID(baseClasses.jniUtilClass,
                                                           SecureUtil_getAppVersion_Method,
                                                           SecureUtil_getAppVersion_Param);
    jstring appVersion = (jstring) env->CallStaticObjectMethod(baseClasses.jniUtilClass,
                                                               methodGetAppVersion);
    string strAppVersion = jstring2String(env, appVersion);

    jmethodID methodGetChannel = env->GetStaticMethodID(baseClasses.jniUtilClass,
                                                        SecureUtil_getChannel_Method,
                                                        SecureUtil_getChannel_Param);
    jstring channel = (jstring) env->CallStaticObjectMethod(baseClasses.jniUtilClass,
                                                            methodGetChannel);
...
void initDebug(JNIEnv *env) {
    ...
    for (int i = 0; i < 30; i++) {
        if (name[i] == '\0') {// c/c++中 '\0'字符串结束符 ASCLL中时NULL
            break;
        }
        if (name[i] == '.') {
            name[i] = '/';
...
void showMeg(JNIEnv *env, jstring str) {
    jmethodID methodshowMeg = env->GetStaticMethodID(baseClasses.jniUtilClass,
                                                       SecureUtil_showMeg_Method,
                                                       SecureUtil_showMeg_Param);
    env->CallStaticVoidMethod(baseClasses.jniUtilClass, methodshowMeg, str);
}
